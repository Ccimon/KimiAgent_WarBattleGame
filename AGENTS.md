...
- `src/net.ts` —— P2P 联机层:`hostRoom`/`joinRoom` 封装 PeerJS,房间码即 Peer ID(前缀 `tower-battle-`);消息协议:客→主 `{t:'cmd',from,to}`,主→客 `{t:'start',levelIndex}` 与 `{t:'snap',towers,squads,phase}` 快照(只带动态字段,几何由客人端按 levelIndex 本地重建)。
- `src/game.ts` —— 纯逻辑层(无 DOM 依赖):`Owner`('player'/'ai1'/'ai2'/'neutral')、`GameState`/`Tower`/`Squad` 类型,`createGame`、`sendUnits`、`towerAt`、`update(dt)`;内置产兵速率/上限表、等级推导(`levelOf`,兵力 >=35 为 3 级、>=15 为 2 级)、AI 性格表 `AI_PERSONALITIES`(莽夫/龟缩/猎强,`createGame` 时为每个 AI 势力随机分配;性格决定决策间隔、出兵门槛、目标选择,猎强专打总兵力最强的对手)、每势力独立计时的 `aiTick`、胜负判定(玩家全灭判负,所有 AI 势力全灭判胜)。
- `src/levels.ts` —— 关卡数据 `LEVELS`(`LevelDef` = 名称 + 初始塔数组),目前共 6 关(前 3 关两方,后 3 关三方会战)。
- `src/render.ts` —— Canvas 绘制层:`draw(ctx, state, drag)`,只读 `GameState`,含拖拽预览。
...
单机无网络请求;联机模式会连接 PeerJS 公共云(仅交换 WebRTC 信令和房间码,房间码不保密,知道即可加入)。无用户输入持久化(localStorage 未使用)、无密钥。注意 `package.json` 中 `"private": true`,不要误发布到 npm。
